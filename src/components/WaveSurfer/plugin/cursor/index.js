...
export default class CursorPlugin {
  ...
  _onMousemove = (e) => {
    ...
    let flip =
      bbox.right <
      event.clientX + this.displayTime.getBoundingClientRect().width;
  ...
  _onReady() {
    ...
    this.cursor = this.util.withOrientation(
      this.wrapper.appendChild(document.createElement("cursor")),
    ...
    if (this.params.showTime) {
      this.showTime = this.util.withOrientation(
        this.wrapper.appendChild(document.createElement("showTitle")),
        this.wavesurfer.params.vertical
      );
      this.style(
        this.showTime,
      ...
      this.displayTime = this.util.withOrientation(
        this.showTime.appendChild(document.createElement("div")),
        this.wavesurfer.params.vertical
      );
  ...
  updateCursorPosition(xpos, ypos, flip = false) {
    ...
    if (this.params.showTime) {
      ...
      const timeValue =
        Math.max(
          0,
          ((xpos - this.wrapper.scrollLeft) / elementWidth) * duration
        ) + scrollTime;
